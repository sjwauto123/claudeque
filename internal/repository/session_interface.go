...
type SessionRepository interface {
	// SaveSession 保存会话元数据
	SaveSession(userID uint, username string, isRoot bool, createdAt time.Time) error

	// GetSessionInfo 获取会话信息
	GetSessionInfo(userID uint) (*SessionInfo, error)
	...
	UpdateLastUsed(userID uint, lastUsedAt time.Time) error

	// SaveUserCredentials 保存用户SSH凭证（用于终端重连）
	SaveUserCredentials(userID uint, username, password string, expiresAt time.Time) error

	// GetUserCredentials 获取用户SSH凭证
	GetUserCredentials(userID uint) (*UserCredentials, error)
