...
type FileListRequest struct {
	...
	TargetUserID int    `form:"target_user_id" json:"target_user_id"` // 目标用户ID (管理员查看他人目录时使用)
}

// DiskUsageRequest 计算目录磁盘占比和大小
type DiskUsageRequest struct {
	Path string `form:"path" json:"path" binding:"required"` // 目录路径
}
...
type DownloadRequest struct {
	Path string `form:"path" json:"path" binding:"required"` // 要下载的文件路径
}
